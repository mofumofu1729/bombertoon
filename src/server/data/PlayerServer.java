...
public class PlayerServer {
    ...
    public void putBomb(FieldServer[][] field) {
        if (bombCount <= 3) {
            ...
            if (xTmp >= 0 && xTmp < BattleServer.FIELDWIDTH && yTmp >= 0
                    && yTmp < BattleServer.FIELDHEIGHT && !(field[yTmp][xTmp].isExistBomb)
    ...
    public void move(Direction move_dir, FieldServer[][] field) {
        ...
        if (yTmp >= 0 && yTmp <= (BattleServer.FIELDHEIGHT - 1)
                && xTmp <= (BattleServer.FIELDWIDTH - 1) && xTmp >= 0
                && !(field[yTmp][xTmp].isExistBomb) // 先に添字範囲判定していないと配列の添字範囲超える
                && !(field[yTmp][xTmp].isExistHuman)
                && (field[yTmp][xTmp].status == Status.NOTHING
                        || ((field[yTmp][xTmp].status == Status.MUTEKI)
                                && (field[yTmp][xTmp].color == this.team))
